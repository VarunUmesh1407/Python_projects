...
def calculate_hours(employee_id):
    ...
    get_data_query = "SELECT login, logout FROM work_hours WHERE date = CURRENT_DATE LIMIT 0, 1 AND employee_id = %s"
    parameters_data = employee_id
    update_hours = "UPDATE work_hours SET hours = %s WHERE date = %s",
    ...
    parameters_hours = (stunden_with_break, system_date)
